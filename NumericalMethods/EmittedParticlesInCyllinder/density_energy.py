import emitting_particles as ep
import search_sigmas
import numpy as np
import math
...
for i in range(ep.n):
    ...
    while ep.isInArea(x1, y1, z1) and np.random.random() <= prob_c:
        ...
        # cos_teta = np.random.uniform(-1, 1)
        ...
        for j in range(n):
            ...
            R_bottom_side = np.sqrt(pow(x1 - x_bottom_side, 2) + pow(y1 - y_bottom_side, 2) + pow(z1 - z_sides[j], 2))
            R_top_side = np.sqrt(pow(x1 - x_top_side, 2) + pow(y1 - y_top_side, 2) + pow(z1 - z_sides[j], 2))
            density_left_side[j] = density_left_side[j] + k * W * np.exp(-sigmas[1] * R_bottom_side) / pow(R_bottom_side, 2)
            density_right_side[j] = density_right_side[j] + k * W * np.exp(-sigmas[1] * R_top_side) / pow(
                R_top_side, 2)
